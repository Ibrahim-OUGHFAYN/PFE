...
const AdminPlaces = () => {
  ...
  const [loading, setLoading] = useState(true); // State to manage loading status for places
  const [deleteLoading, setDeleteLoading] = useState(false); // State to manage loading status for deletion
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        setPlaces(res.data);
        setLoading(false); // Data is loaded, turn off the loading state
      })
      .catch((err) => {
        console.error("Erreur lors de la récupération :", err);
        setLoading(false); // In case of error, turn off the loading state
      });
  }, []);
