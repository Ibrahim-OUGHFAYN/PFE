    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <div className="flex items-center gap-1 cursor-pointer">
          <img
            src="http://localhost:5000/assets/defaultUserImg.png"
            alt="Profile"
            className="w-10 h-10 rounded-full"
          />
        </div>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-48 bg-white shadow-lg rounded-md p-2 border border-gray-200">
        <Button
  ...
  return (
    <nav className="fixed top-2 left-1/2 transform -translate-x-1/2 z-50 flex items-center justify-between p-4 shadow-md bg-gray-100 rounded-full w-[90%] max-w-15xl">
      ...
      <button className="lg:hidden p-2" onClick={toggleMenu}>
        <img src={isMenuOpen ? cancel : MenuIcon} alt="Menu" className="w-6 h-6" />
      </button>
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-16 left-0 right-0 bg-white shadow-lg flex flex-col items-center py-4 gap-2 rounded-lg border-1 border-red-500">
          ...
          <Button variant="ghost" onClick={() => handleScroll("contact")} className="hover:text-red-500 cursor-pointer">Contact</Button>
          {user ? (
            <ProfileDropdown />
          ) : (
            <>
